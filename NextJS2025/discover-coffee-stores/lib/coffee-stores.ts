...
const transformCoffeeData = (result: MapboxType) => {
  return {
    ...
    address: result.properties?.full_address || 'Not Found',
...
export const fetchCoffeeStore = async (id: string) => {
  try {
    const response = await fetch (
      `https://api.mapbox.com/search/searchbox/v1/retrieve/${id}?language=en&session_token=012a7dd2-9523-4511-894e-05d63e7096d8&access_token=${process.env.MAPBOX_API}`
    );
    const data = await response.json();
    ...
    // console.log('Coffeestores from lib coffee-stores', coffeeStore)
